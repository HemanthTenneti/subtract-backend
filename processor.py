...
# Updated connection with SSL configuration
mongo = MongoClient(
    "mongodb+srv://hemanthtenneti:SE6CR8ptI8PyQ8Lf@subtract.ztnydst.mongodb.net/?retryWrites=true&w=majority&appName=subtract&tls=true&tlsAllowInvalidCertificates=true",
    ssl=True,
    ssl_cert_reqs=ssl.CERT_NONE,  # Try with this setting first
    tls=True
)
db = mongo["subtract"]
collection = db.get_collection("summarized_videos")

def download_audio(url):
    name = f"{uuid.uuid4().hex}.mp3"
    print("Downloading...")
    with yt_dlp.YoutubeDL({'extract_audio': True, 'format': 'bestaudio', 'outtmpl': name}) as ydl:
...
def process_url(url):
    if collection is None:
        db.create_collection("summarized_videos")
        print("Created new MongoDB collection: 'summarized_videos'")

    doc = collection.find_one({"url": url})
    if doc:
        print("Found in DB.")
        return doc["summary"], doc["transcript"]
